...
-- optional e _ Empty = e; optional _ f (Full a) = f a
...
setFocus n (ListZipper l _ r) =  ListZipper l n r
...
findLeft f z@(ListZipper (h:.t) x r) = if f x
		  							 then IsZ z
							         else findLeft f (ListZipper t h (x:.r)) 
-- findLeft' = 
-- case break p (x:.l) of 
-- 	(r', )

-- then IsZ (ListZipper t h (x:.r))
...
findRight f z@(ListZipper l x (h:.t)) = if f x
		  							 then IsZ z
							         else findRight f (ListZipper (x:.l) h t)
...
moveLeft (ListZipper l x r) = IsZ (ListZipper l x r) 
...
moveRight (ListZipper l x r) = IsZ (ListZipper l x r)
